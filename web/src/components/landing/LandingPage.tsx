...
function HeroSection() {
  return (
    <section className="relative pt-32 pb-20 overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[600px] bg-gradient-to-b from-teal-50 to-transparent rounded-full blur-3xl opacity-60" />

      <div className="section-container relative">
        <div className="max-w-3xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            <Badge variant="outline" className="mb-6 py-1.5 px-4 text-xs font-medium bg-white shadow-sm border-slate-200">
          ...
          >
            ...
            <span className="text-teal-600">India&apos;s Gig Workers</span>
          </motion.h1>
          ...
          >
            ...
            <Link href="/dashboard/map">
              <Button variant="outline" size="lg" className="rounded-full px-8">
                Explore Risk Map <ArrowRight className="h-4 w-4 ml-1.5" />
              </Button>
            </Link>
          </motion.div>
          ...
          >
            ...
            <div className="flex gap-3 flex-wrap justify-center">
              <div className="inline-flex items-center gap-2 bg-slate-900 text-slate-300 rounded-full px-4 py-2 text-sm">
                <span>Live trigger monitoring across 5 cities</span>
              </div>
              <div className="inline-flex items-center gap-2 bg-slate-900 text-slate-300 rounded-full px-4 py-2 text-sm">
                <span>Automated payouts in under 2 minutes</span>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        >
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { value: "15,000+", label: "Workers Protected", icon: Users },
              { value: "98%", label: "Auto-Processed", icon: Zap },
              { value: "< 2 min", label: "Average Payout", icon: Clock },
              { value: "99.2%", label: "Fraud Detection", icon: Shield },
            ].map((stat) => (
              <div key={stat.label} className="text-center p-4 rounded-xl bg-white border border-slate-200 shadow-sm">
                <stat.icon className="h-4 w-4 text-teal-600 mx-auto mb-2" />
...
// ─── Tech Logos ─────────────────────────────────────────────────
function TechRow() {
  const techs = ["Supabase", "XGBoost", "OpenWeatherMap", "Leaflet", "SHAP", "Isolation Forest", "FastAPI"];
  return (
    <section className="py-10 border-y border-slate-100">
      <div className="section-container">
        <div className="flex items-center justify-center gap-8 md:gap-12 flex-wrap">
          {techs.map((t) => (
            <span key={t} className="text-sm font-medium text-slate-400 hover:text-slate-600 transition-colors cursor-default">{t}</span>
          ))}
        </div>
      </div>
    </section>
    ...
    <section className="section-padding" id="features">
      <div className="section-container">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {features.map((f) => (
            <div key={f.title} className="group p-6 rounded-xl border border-slate-200 bg-white hover:border-teal-200 hover:shadow-md transition-all duration-200">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            {triggers.map((t) => (
              <div key={t.name} className="grid grid-cols-4 px-6 py-4 items-center border-b border-slate-100 last:border-b-0 hover:bg-slate-50 transition-colors">
                ...
                <span className="text-sm font-semibold text-teal-600">{t.payout} daily earnings</span>
    ...
    <section className="section-padding bg-slate-50" id="pricing">
      <div className="section-container">
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {PLAN_TIERS.map((plan) => (
            <div key={plan.id} className={`relative p-6 rounded-xl border bg-white transition-all ${plan.popular ? "border-teal-300 shadow-md ring-1 ring-teal-200" : "border-slate-200 shadow-sm hover:border-slate-300"}`}>
              {plan.popular && (
...
function CTASection() {
  return (
    <section className="section-padding">
      <div className="section-container">
        <div className="max-w-3xl mx-auto bg-slate-900 rounded-2xl p-10 md:p-14 text-center">
